...
if(($_GET['acao'] == 'incluir') || ($_GET['acao'] == 'alterar')) {
	...
	$uf = verifica_vazio(strtoupper($_POST['uf']), "NI");
	$status = $_POST['status'];
}

if($_GET['acao'] == 'incluir') {
	// REALIZA A INCLUSAO
	$sql_inclui = "INSERT INTO estado (nome_estado,uf_estado,status) VALUES ('".$nome."','".$uf."','".$status."')";
	$query_inclui = $pdo->prepare($sql_inclui);
  $query_inclui->execute();
	
	// VERIFICA SE DEU ERRO
	if($query_inclui) {
		salvar_log($_SESSION["id_usuario"],"incluir estado",$sql_inclui,$pdo);
		redireciona("gerenciar.php?aviso=ok&msg=Informaçães do estado salvas com sucesso.","_self");
	} else {
...
if($_GET['acao'] == 'alterar') {
	...
	if($_GET['tipo'] == 'alterar') {
		// Coletando dados
		$sql_dados_alterar = "SELECT estado.nome_estado, estado.uf_estado, estado.status FROM estado WHERE estado.codigo_estado = '".$id."'";
		$query_dados_alterar = $pdo->query($sql_dados_alterar);
		$executa_dados_alterar = $query_dados_alterar->fetch( PDO::FETCH_ASSOC );
		
	} else {
		// REALIZA A ALTERACAO
		$sql_altera = "UPDATE estado SET nome_estado = '".$nome."', uf_estado = '".$uf."', status = '".$status."' WHERE codigo_estado = '".$id."'";
		$query_altera = $pdo->prepare($sql_altera);
  	$query_altera->execute();
		
		// VERIFICA SE DEU ERRO
		if($query_altera) {
			salvar_log($_SESSION["id_usuario"],"alterar estado",$sql_altera,$pdo);
			redireciona("gerenciar.php?aviso=ok&msg=Informações do estado salvas com sucesso.","_self");
		} else {
...
<body>
<div class="titulo-sessao-azul">Estado</div>
    ...
    <td colspan="2">
      <div id="tabFormulario" class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
          <li class="TabbedPanelsTab" tabindex="0">Informa&ccedil;&otilde;es do estado</li>
        </ul>
          ...
          <div class="TabbedPanelsContent">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td><input name="nome" type="text" id="nome" value="<?php echo $executa_dados_alterar['nome_estado']; ?>" size="50" maxlength="100" ></td>
              </tr>
              <tr>
                <td class="campo-form">UF</td>
                <td><input name="uf" type="text" id="uf" value="<?php echo $executa_dados_alterar['uf_estado']; ?>" size="2" maxlength="2" ></td>
              </tr>
              <tr>
