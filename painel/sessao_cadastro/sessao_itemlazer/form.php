...
if($_GET['acao'] == 'incluir') {
	// REALIZA A INCLUSAO
	$sql_inclui = "INSERT INTO site_itemlazer (nome,status) VALUES ('".$nome."','".$status."')";
	$query_inclui = @mysql_query($sql_inclui);
	
	// VERIFICA SE DEU ERRO
	if($query_inclui) {
		salvar_log($_SESSION["id_usuario"],"incluir item de lazer",$sql_inclui);
		redireciona("gerenciar.php?aviso=ok&msg=Informações do item de lazer salvas com sucesso.","_self");
	} else {
...
if($_GET['acao'] == 'alterar') {
	...
	if($_GET['tipo'] == 'alterar') {
		// Coletando dados
		$sql_dados_alterar = "SELECT site_itemlazer.nome, site_itemlazer.status FROM site_itemlazer WHERE site_itemlazer.id_itemlazer = '".$id."'";
		$query_dados_alterar = @mysql_query($sql_dados_alterar);
		$executa_dados_alterar = @mysql_fetch_assoc($query_dados_alterar);
	} else {
		// REALIZA A ALTERACAO
		$sql_altera = "UPDATE site_itemlazer SET nome = '".$nome."', status = '".$status."' WHERE id_itemlazer = '".$id."'";
		$query_altera = @mysql_query($sql_altera);
		
		// VERIFICA SE DEU ERRO
		if($query_altera) {
			salvar_log($_SESSION["id_usuario"],"alterar item de lazer",$sql_altera);
			redireciona("gerenciar.php?aviso=ok&msg=Informações do item de lazer salvas com sucesso.","_self");
		} else {
...
<body>
<div class="titulo-sessao-azul">Item de lazer</div>
    ...
    <td colspan="2">
      <div id="tabFormulario" class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
          <li class="TabbedPanelsTab" tabindex="0">Informa&ccedil;&otilde;es do item de lazer</li>
        </ul>
