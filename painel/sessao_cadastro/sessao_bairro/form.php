...
if($_GET['acao'] == 'incluir') {
	// REALIZA A INCLUSAO
	$sql_inclui = "INSERT INTO bairro (codigo_cidade,nome_bairro,status) VALUES ('".$codigo_cidade."','".$nome."','".$status."')";
	$query_inclui = $pdo->prepare($sql_inclui);
  $query_inclui->execute();
	
	// VERIFICA SE DEU ERRO
	if($query_inclui) {
		salvar_log($_SESSION["id_usuario"],"incluir bairro",$sql_inclui,$pdo);
		redireciona("gerenciar.php?aviso=ok&msg=Informações do bairro salvas com sucesso.","_self");
	} else {
...
if($_GET['acao'] == 'alterar') {
	...
	if($_GET['tipo'] == 'alterar') {
		// Coletando dados
		$sql_dados_alterar = "SELECT estado.codigo_estado, bairro.codigo_cidade, bairro.nome_bairro, bairro.status FROM bairro, cidade, estado WHERE bairro.codigo_cidade = cidade.codigo_cidade AND cidade.codigo_estado = estado.codigo_estado AND bairro.codigo_bairro = '".$id."'";
		$query_dados_alterar = $pdo->query($sql_dados_alterar);
		$executa_dados_alterar = $query_dados_alterar->fetch( PDO::FETCH_ASSOC );
	} else {
		// REALIZA A ALTERACAO
		$sql_altera = "UPDATE bairro SET codigo_cidade = '".$codigo_cidade."', nome_bairro = '".$nome."', status = '".$status."' WHERE codigo_bairro = '".$id."'";
		$query_altera = $pdo->prepare($sql_altera);
  	$query_altera->execute();
		
		// VERIFICA SE DEU ERRO
		if($query_altera) {
			salvar_log($_SESSION["id_usuario"],"alterar bairro",$sql_altera,$pdo);
			redireciona("gerenciar.php?aviso=ok&msg=Informações do bairro salvas com sucesso.","_self");
		} else {
...
<body>
<div class="titulo-sessao-azul">Bairro</div>
    ...
    <td colspan="2">
      <div id="tabFormulario" class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
          <li class="TabbedPanelsTab" tabindex="0">Informa&ccedil;&otilde;es do bairro</li>
        </ul>
          ...
          <div class="TabbedPanelsContent">
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
                ...
                <td>
                    <select name="codigo_estado" id="codigo_estado" onchange="carregaCidade(this.value);">
              ...
              <tr>
                ...
                <td>
                    <select name="codigo_cidade" id="codigo_cidade">
										<?php foreach($resultado_cidade AS $cidade):?>
										<option value="<?php echo $cidade["codigo_cidade"];?>" <?php if($executa_dados_alterar['codigo_cidade'] == $cidade["codigo_cidade"]) { echo 'selected="selected"'; } ?>><?php echo $cidade["nome_cidade"];?></option>
										<?php endforeach;?>
                    </select>
                </td>
              </tr>
              <tr>
                ...
                <td><input name="nome" type="text" id="nome" value="<?php echo $executa_dados_alterar['nome_bairro']; ?>" size="50" maxlength="100" ></td>
              </tr>
              <tr>
...
function carregaCidade(codigo_estado){
    ...
    action = '../../sessao_revendedor/carregaCidade.php';
    $.ajax({
        type: "POST",
        url: action,
        data: parametros,
        beforeSend: function() {
            $('#codigo_cidade').empty().append('<option value="0">Aguarde...</option>');
        },        
        success: function(txt) {  
            $('#codigo_cidade').empty().append(txt);
        },
        error: function(txt) {
            $('#codigo_cidade').empty().append('<option value="0">Erro!</option>');
        }
    });
}
