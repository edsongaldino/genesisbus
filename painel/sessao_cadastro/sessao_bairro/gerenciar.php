...
if($palavra_chave) {
	...
	for($i=0;$i<$total_palavra_chaves;$i++) {
		$condicao_consulta .= "bairro.nome LIKE '%".$palavra_chaves[$i]."%'";
...
// Coleta os dados
$sql_gerenciar = "SELECT bairro.codigo_bairro, estado.nome_estado, cidade.nome_cidade, bairro.nome_bairro, bairro.status FROM bairro, cidade, estado WHERE bairro.status <> 'E' AND bairro.codigo_cidade = cidade.codigo_cidade AND cidade.codigo_estado = estado.codigo_estado AND bairro.status <> 'E'".$condicao_consulta." ORDER BY bairro.nome_bairro";
...
<body>
<div class="titulo-sessao-azul">Bairros</div>
...
<form id="form1" name="form1" method="get" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=consultar">
  <tr>
    <td><input type="button" name="button" id="button" onclick="javascript: window.open('form.php','_self');" value="Incluir novo bairro" /></td>
  </tr>
  <tr>
    <td>
    ...
    <td>
        <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
          ...
          <tr bgcolor="<?php if(is_int($i/2) != 1) { echo "#EDEFF3"; } else { echo "#FFFFFF"; }; ?>">
            ...
            <td><div align="center"><?php echo $executa_gerenciar['codigo_bairro']; ?></div></td>
            <td><?php echo $executa_gerenciar['nome_estado']; ?></td>
            <td><?php echo $executa_gerenciar['nome_cidade']; ?></td>
            <td><?php echo $executa_gerenciar['nome_bairro']; ?></td>
            <td><div align="center"><a href="form.php?acao=alterar&tipo=alterar&id=<?php echo $executa_gerenciar['codigo_bairro']; ?>" target="_self"><img src="/painel/imagens/icone/icone_alterar.png" title="Alterar" width="16" height="16" border="0" /></a></div></td>
            <td><div align="center"><a href="javascript: confirma_acao('Tem certeza que deseja excluir <?php echo $executa_gerenciar['nome']; ?>?','excluir.php?id=<?php echo $executa_gerenciar['codigo_bairro']; ?>');" target="_self"><img src="/painel/imagens/icone/icone_excluir.gif" title="Excluir" width="16" height="16" border="0" /></a></div></td>
          </tr>
          <?php $i += 1; } ?>
        </table>
    </td>
