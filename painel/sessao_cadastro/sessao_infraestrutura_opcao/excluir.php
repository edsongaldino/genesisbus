...
if($query_exclui) {
	// Log
	salvar_log($_SESSION["id_usuario"],"excluir opção infraestrutura",$sql_exclui);
		
	echo "<script>";
	echo "open('gerenciar.php?aviso=ok&msg=Opção da infraestrutura excluída com sucesso.','_self');";
	echo "</script>";
} else {
	// Log
	...
	echo "<script>";
	echo "open('gerenciar.php?aviso=erro&msg=Não foi possível exclui a opção da infraestrutura.','_self');";
	echo "</script>";
}
?>