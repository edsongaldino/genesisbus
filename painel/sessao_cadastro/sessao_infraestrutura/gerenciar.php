...
if($palavra_chave) {
	...
	for($i=0;$i<$total_palavra_chaves;$i++) {
		$condicao_consulta .= "site_infraestruturas.nome LIKE '%".$palavra_chaves[$i]."%'";
...
// Coleta os dados
$sql_gerenciar = "SELECT site_infraestruturas.id_infraestrutura, site_infraestruturas.nome, site_infraestruturas.status FROM site_infraestruturas WHERE site_infraestruturas.status <> 'E'".$condicao_consulta." ORDER BY site_infraestruturas.nome";
...
<body>
<div class="titulo-sessao-azul">Infraestruturas</div>
...
<form id="form1" name="form1" method="get" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=consultar">
  <tr>
    <td><input type="button" name="button" id="button" onclick="javascript: window.open('form.php','_self');" value="Incluir nova infraestrutura" /></td>
  </tr>
  <tr>
    <td>
    ...
    <td>
        <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#CCCCCC">
          ...
          <tr bgcolor="<?php if(is_int($i/2) != 1) { echo "#EDEFF3"; } else { echo "#FFFFFF"; }; ?>">
            ...
            <td><div align="center"><?php echo $executa_gerenciar['id_infraestrutura']; ?></div></td>
            <td><?php echo $executa_gerenciar['nome']; ?></td>
            <td><div align="center"><a href="form.php?acao=alterar&tipo=alterar&id=<?php echo $executa_gerenciar['id_infraestrutura']; ?>" target="_self"><img src="/paineladm/imagens/icone/icone_alterar.png" title="Alterar" width="16" height="16" border="0" /></a></div></td>
            <td><div align="center"><a href="javascript: confirma_acao('Tem certeza que deseja excluir <?php echo $executa_gerenciar['nome']; ?>?','excluir.php?id=<?php echo $executa_gerenciar['id_infraestrutura']; ?>');" target="_self"><img src="/paineladm/imagens/icone/icone_excluir.gif" title="Excluir" width="16" height="16" border="0" /></a></div></td>
          </tr>
          <?php $i += 1; } ?>
        </table>
    </td>
