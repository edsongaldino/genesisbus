...
if($_GET['acao'] == 'incluir') {
	// REALIZA A INCLUSAO
	$sql_inclui = "INSERT INTO site_infraestruturas (nome,status) VALUES ('".$nome."','".$status."')";
	$query_inclui = @mysql_query($sql_inclui);
	
	// VERIFICA SE DEU ERRO
	if($query_inclui) {
		salvar_log($_SESSION["id_usuario"],"incluir infraestrutura",$sql_inclui);
		redireciona("gerenciar.php?aviso=ok&msg=Informações da infraestrutura salvas com sucesso.","_self");
	} else {
...
if($_GET['acao'] == 'alterar') {
	...
	if($_GET['tipo'] == 'alterar') {
		// Coletando dados
		$sql_dados_alterar = "SELECT site_infraestruturas.nome, site_infraestruturas.status FROM site_infraestruturas WHERE site_infraestruturas.id_infraestrutura = '".$id."'";
		$query_dados_alterar = @mysql_query($sql_dados_alterar);
		$executa_dados_alterar = @mysql_fetch_assoc($query_dados_alterar);
	} else {
		// REALIZA A ALTERACAO
		$sql_altera = "UPDATE site_infraestruturas SET nome = '".$nome."', status = '".$status."' WHERE id_infraestrutura = '".$id."'";
...
<body>
<div class="titulo-sessao-azul">Infraestrutura</div>
    ...
    <td colspan="2">
      <div id="tabFormulario" class="TabbedPanels">
        <ul class="TabbedPanelsTabGroup">
          <li class="TabbedPanelsTab" tabindex="0">Informa&ccedil;&otilde;es da infraestrutura</li>
        </ul>
