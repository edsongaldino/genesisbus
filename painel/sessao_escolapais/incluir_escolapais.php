...
if($_GET['acao'] == 'incluir') {
	...
	// Inclui
	$sql_inclui = "INSERT INTO escola_pais (titulo,data,arquivo,texto,status) VALUES ('".$titulo."','".$data."','".$nome_arquivo."','".$texto."','".$status."')";
	...
	if($query_inclui) {
		...
		echo "<script>";
		echo "open('gerenciar_escolaspais.php?aviso=ok&msg=Escola de pais incluída com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível incluir a escola de pais.','_self');";
		echo "</script>";
	}
}
?>
...
<div class="titulo-sessao-azul">Incluir nova escola de pais</div>
...
<form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=incluir" enctype="multipart/form-data" onsubmit="javascript: this.button.value='Aguarde, carregando...'; this.button.disabled='disabled';">
  <tr>
    <td colspan="2">
      <div id="incluir_escola_pais" class="TabbedPanels">
  ...
  <tr>
    ...
    <td><div align="right"><button type="submit" name="button" id="button">Salvar nova escola de pais</button></div></td>
  </tr>
</form>
</table>
<script type="text/javascript">
var incluir_escola_pais = new Spry.Widget.TabbedPanels("incluir_escola_pais");
</script>
</body>
</html>