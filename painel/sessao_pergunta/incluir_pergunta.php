...
if($_GET['acao'] == 'incluir') {
	...
	// Inclui
	$sql_inclui = "INSERT INTO perguntas_resposta (pergunta,resposta,status) VALUES ('".$pergunta."','".$resposta."','".$status."')";
	$query_inclui = mysql_query($sql_inclui);

	// VERIFICA SE DEU ERRO
	if($query_inclui) {
		// Log
		salvar_log($_SESSION["id_usuario"],"incluir pergunta e resposta",$sql_inclui);

		echo "<script>";
		echo "open('gerenciar_perguntas.php?aviso=ok&msg=Pergunta e resposta incluída com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível incluir a pergunta e resposta.','_self');";
		echo "</script>";
	}
}
?>
...
<div class="titulo-sessao-azul">Incluir nova pergunta e resposta</div>
...
<form id="form1" name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?acao=incluir" enctype="multipart/form-data" onsubmit="javascript: this.button.value='Aguarde, carregando...'; this.button.disabled='disabled';">
  <tr>
    <td colspan="2">
      <div id="incluir_pergunta_resposta" class="TabbedPanels">
          ...
          <div class="TabbedPanelsContent">
            <table width="100%" border="0" cellspacing="3" cellpadding="0">
              <tr>
                ...
                <td><textarea name="pergunta" cols="50" rows="5" id="pergunta"></textarea></td>
              </tr>
              <tr>
                ...
                <script src="../ferramentas/editor/nicEdit.js" type="text/javascript"></script>
                    ...
                    <textarea name="resposta" cols="60" rows="10" id="resposta"></textarea>
  ...
  <tr>
    ...
    <td><div align="right"><input type="submit" name="button" id="button" value="Salvar nova pergunta e resposta" /></div></td>
  </tr>
</form>
</table>
<script type="text/javascript">
var incluir_pergunta_resposta = new Spry.Widget.TabbedPanels("incluir_pergunta_resposta");
</script>
</body>
</html>