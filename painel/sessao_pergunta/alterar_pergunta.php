...
if($_GET['acao'] == 'alterar') {
	...
	if($query_alterar) {
		...
		echo "<script>";
		echo "open('gerenciar_perguntas.php?aviso=ok&msg=Informações da pergunta e resposta alterada com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível alterar as informações da pergunta e resposta.&id_pergunta_resposta=".base64_encode($id_pergunta_resposta)."','_self');";
		echo "</script>";
	}
}
?>
