...
if($_GET['acao'] == 'incluir') {
	...
	if(($imagem["type"] == "image/jpeg") || ($imagem["type"] == "image/pjpeg")) {
		...
		if($ftp_upload_imagem) {
			...
			// Log
			salvar_log($_SESSION["id_usuario"],"erro","não foi possivel incluir o imagem ".$nome_imagem);
		}
	}
	...
	if($query_inclui) {
		...
		echo "<script>";
		echo "open('gerenciar_folhasbrasilis.php?aviso=ok&msg=Arquivo (folha brasilis) incluído com sucesso.','_self');";
		echo "</script>";
	} else {
		// Log
		...
		echo "<script>";
		echo "open('".$_SERVER['SCRIPT_NAME']."?aviso=erro&msg=Não foi possível incluir o arquivo (folha brasilis).','_self');";
		echo "</script>";
	}
}
?>
